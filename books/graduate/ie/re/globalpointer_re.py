...
epochs = 7
learning_rate = 1e-5
# config_path = os.path.join(script_dir, '../../util/model_hub/chinese_L-12_H-768_A-12/bert_config.json')
# checkpoint_path = os.path.join(script_dir, '../../util/model_hub/chinese_L-12_H-768_A-12/bert_model.ckpt')
# dict_path = os.path.join(script_dir, '../../util/model_hub/chinese_L-12_H-768_A-12/vocab.txt')
config_path = os.path.join(script_dir, '../../util/model_hub/roberta_zh_l12/bert_config.json')
checkpoint_path = os.path.join(script_dir, "../../util/model_hub/roberta_zh_l12/bert_model.ckpt")
dict_path = os.path.join(script_dir, '../../util/model_hub/roberta_zh_l12/vocab.txt')
...
# 加载数据集
train_data = load_data(os.path.join(script_dir, '../../util/data_hub/CMeIE/CMeIE_train.jsonl'))
valid_data = load_data(os.path.join(script_dir, '../../util/data_hub/CMeIE/CMeIE_dev.jsonl'))
test_data = load_data(os.path.join(script_dir, '../../util/data_hub/CMeIE/CMeIE_test.jsonl'))
predicate2id, id2predicate = {}, {}

with open(os.path.join(script_dir, '../../util/data_hub/CMeIE/53_schemas.jsonl'), encoding="utf8") as f:
    for l in f:
        l = json.loads(l)
...
def predict(data):
    for i, d in enumerate(data):
        R = set([SPO(spo) for spo in extract_spoes(d['text'] if not isinstance(d, str) else d)])
        print(d['text'] if not isinstance(d, str) else d)
        print(R)
        if i == 3:
            break
...
def evaluate(data):
    ...
    f = open(os.path.join('dev_cmedee_globakpointer.json'), 'w', encoding='utf-8')
    pbar = tqdm()
    for d in data:
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if f1 >= self.best_val_f1:
            self.best_val_f1 = f1
            model.save_weights(os.path.join('../../util/checkpoint/re/best_model_cmedee_globalpointer.weights'))
...
if __name__ == '__main__':
    ...
    if do_train:
        ...
        model.load_weights(os.path.join("../../util/checkpoint/re/best_model_cmedee_globalpointer.weights"))
        model.fit(
    ...
    if do_predict:
        ...
        # import pandas as pd
        #
        # json_file = pd.read_excel(excel_file, na_values="NaN")
        # content = []
        # for item in json_file["content"]:
        #     content.append(item)
        # # print(content)
